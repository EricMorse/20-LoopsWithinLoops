...
def draw_L(window, circle, r, c):
    ...
    """
    # ------------------------------------------------------------------
    # DONR: 2. Implement and test this function.
...
def draw_wall_on_right(rectangle, n, window):
    ...
    original_ul = rectangle.get_upper_left_corner()
    original_lr = rectangle.get_lower_right_corner()

    for i in range(n):  # Loop through the rows
        for j in range(i + 1):
            ul = rg.Point(original_ul.x + width * (j - i), original_ul.y + height * i)
            lr = rg.Point(original_lr.x + width * (j - i), original_lr.y + height * i)
